...
export function FloorCard({
  ...
  /*
   * The hero. On day one there is no measurement to choose from, so the
   * card says what the first recording is FOR rather than inventing a
   * reason for it, which is the same rule the trait cards follow.
   */
  const prompt = dayOne
  ...
  return (
    <div className="elev-2 rounded-sheet border border-card-edge bg-raised p-5">
      ...
      <h2 className="font-display mt-2.5 text-title leading-tight">{prompt}</h2>
       ...
       */}
      {chosen && !dayOne && (
        <p className="mt-2 text-body text-stone-500">{chosen.item.tips[0]}</p>
      )}
