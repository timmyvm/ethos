...
export function ErrorState({
  ...
  return (
    <div
      role="alert"
      className={`rounded-[14px] border border-edge bg-raised p-5 ${className}`}
    >
      ...
      {onRetry && (
        <button
          type="button"
          onClick={onRetry}
          className="press font-display mt-3 min-h-11 w-full rounded-[10px] border border-stone-200 bg-surface px-4 py-2.5 text-[13px] font-bold hover:bg-sand"
        >
          {retryLabel}
        </button>
      )}
